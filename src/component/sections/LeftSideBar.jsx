...
function LeftSideBar({ onFilterChange }) {
  ...
  const { setSearchPopup, setShowFavoritesPopup, favoriteItem } = useContext(BookContext);
  ...
  const handleButtonClick = (buttonName) => {
    ...
    if (buttonName === "favorites") setShowFavoritesPopup(true);
  };
    ...
    <>
      ...
      <div className={`fixed left-0 top-16 h-screen p-4 bg-white dark:bg-gray-800 shadow-lg z-40 transition-all ${isOpen ? "w-3/4" : "w-0"} lg:w-1/5 overflow-hidden`}>
        <div className={`${isOpen ? "block" : "hidden"} lg:block mt-4`}>
          <div onClick={() => setSearchPopup(true)} className="relative w-full flex items-center cursor-pointer">
          ...
          >
            ...
            <div className="absolute top-0 left-7 bg-gray-600 dark:bg-white rounded-full w-5 h-5 flex items-center justify-center">
              <span className="dark:text-rose-700 text-white text-xs">{favoriteItem.length}</span>
            </div>
          </div>
        </div>
      </div>
